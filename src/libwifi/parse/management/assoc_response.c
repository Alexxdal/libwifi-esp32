...
int libwifi_parse_assoc_resp(struct libwifi_bss *bss, struct libwifi_frame *frame) {
    ...
    if (frame->len < (frame->header_len + sizeof(struct libwifi_assoc_resp_fixed_parameters) + 2)) {
        return -EINVAL;
    }

    struct libwifi_assoc_resp_fixed_parameters *fixed_params =
        (struct libwifi_assoc_resp_fixed_parameters *) frame->body;
    if (libwifi_check_capabilities(fixed_params->capabilities_information, CAPABILITIES_PRIVACY)) {
        bss->encryption_info |= WEP;
    }

    bss->tags.length =
        (frame->len - (frame->header_len + sizeof(struct libwifi_assoc_resp_fixed_parameters)));
    const unsigned char *tagged_params = frame->body + sizeof(struct libwifi_assoc_resp_fixed_parameters);
